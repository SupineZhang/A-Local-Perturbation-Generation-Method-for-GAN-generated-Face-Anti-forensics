...
def down_layer(chann_in, chann_out, k_size, p_size):
    layer = nn.Sequential(
        nn.MaxPool2d(2),
        conv_layer(chann_in, chann_out, k_size, p_size)
    )
    return layer


def up_layer(chann_in, chann_out, k_size, p_size):
    layer = nn.Sequential(
...
class Model(nn.Module):
    def __init__(self,
        ...
        self.layer5 = down_layer(6, 32, 3, 1)  # 开始做下采样
        self.layer6 = conv_layer(32, 32, 3, 1)
        self.layer7 = up_layer(32, 6, 3, 1)  # 还原做上采样
        # SE模块
        self.layer8 = nn.AdaptiveAvgPool2d(1)
        self.layer9 = se_model(6, 2)
        ...
        # first conv
        layers.update({"stem_conv": ConvBNActivation(in_planes=6,
    ...
    def getfeatures(self, x: Tensor) -> Tensor:
        #自适应残差模块
        out = self.layer1(x)
        origin = out
        # 做减法
        ...
        # SE模块
        b, c, _, _ = out.size()
        y = self.layer8(out).view(b, c)
        y = self.layer9(y).view(b, c, 1, 1)
